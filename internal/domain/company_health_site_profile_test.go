...
func TestPreferredCompanySiteURL(t *testing.T) {
	profile := &CompanySiteProfile{
		WebsiteURL: "https://www.acme.example",
		AboutURL:   "https://www.acme.example/about",
	}
	if got := preferredCompanySiteURL(profile); got != profile.AboutURL {
		t.Errorf("preferredCompanySiteURL(%#v) = %q, want %q", profile, got, profile.AboutURL)
	}

	profile.AboutURL = ""
	if got := preferredCompanySiteURL(profile); got != profile.WebsiteURL {
		t.Errorf("preferredCompanySiteURL(%#v) = %q, want %q", profile, got, profile.WebsiteURL)
	}
}
...
func TestExtractURLHostname(t *testing.T) {
	tests := []struct {
		...
		{
			rawURL: " https://www.acme.example/about ",
			want:   "www.acme.example",
		},
		{
			rawURL: "://invalid",
			want:   "",
	...
	for _, tt := range tests {
		if got := extractURLHostname(tt.rawURL); got != tt.want {
			t.Errorf("extractURLHostname(%q) = %q, want %q", tt.rawURL, got, tt.want)
		}
	}
}
