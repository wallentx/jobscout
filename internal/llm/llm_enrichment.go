...
func buildCompanyHealthLLMPromptWithStats(result *CompanyHealthResult) (string, companyHealthPromptStats) {
	...
	prompt.WriteString("Review only article_context entries in article_reviews; article_reviews must be empty when article_context is absent. ")
	prompt.WriteString("Use the exact article_context url as the article_reviews url. ")
	prompt.WriteString("score_modifier_sources must contain only exact article_context urls. ")
